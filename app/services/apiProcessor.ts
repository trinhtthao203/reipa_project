...
import { IError, IResult } from "@app/commons/interfaces"
...
class APIAccessor {
    ...
    private createAxiosConfig = async (request: IRequest): Promise<any> => {
        ...
        const authState = await LocalStorage.getObject(Constants.StorageKeys.AUTHORIZE_INFO);
        ...
        if (error) {
            ...
            if (response) {
                ...
                if (!Helpers.isNullOrEmpty(response.status)) {
                    ...
                    if (Helpers.isFunction(request.onError)) {
                        const message: string = response.data?.ResponseException?.ExceptionMessage;
                        request.onError({
